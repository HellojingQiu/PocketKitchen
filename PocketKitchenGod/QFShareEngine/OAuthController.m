...
@synthesize nCancelTitle = _nCancelTitle;
@synthesize nTitleView = _nTitleView;

- (void)changeTitle:(NSNotification*)noti{
    self.nTitle = [noti.userInfo objectForKey:@"title"];
    self.title = self.nTitle;
}

- (void)changeCancelTitle:(NSNotification*)noti{
    self.nCancelTitle = [noti.userInfo objectForKey:@"title"];
    self.navigationItem.rightBarButtonItem.title = self.nCancelTitle;
}

- (void)changeTitleView:(NSNotification*)noti{
    self.nTitleView = [noti.userInfo objectForKey:@"title"];
    self.navigationItem.titleView = self.nTitleView;
}
...
- (id)init{
    if (self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeCancelTitle:) name:@"ChangeCancelTitle" object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeTitleView:) name:@"ChangeTitleView" object:nil];
...
- (void)loadView
{
    ...
    if (UIDeviceOrientationIsLandscape(orientation)) {
        _webView.frame = CGRectMake(0, 0, self.view.bounds.size.height, self.view.bounds.size.width);
    }
    ...
    self.title = self.nTitle == nil ? @"登录" : self.nTitle;
    self.navigationItem.rightBarButtonItem.title = self.nCancelTitle == nil ? @"取消" : self.nCancelTitle;
    self.navigationItem.titleView = self.nTitleView;
}
